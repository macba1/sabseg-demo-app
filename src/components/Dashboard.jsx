...
const sevColors = {
  Alta: { bg: '#2d1215', text: '#f87171', border: '#991b1b' },
  Media: { bg: '#2d2212', text: '#fbbf24', border: '#92400e' },
  Baja: { bg: '#122d1f', text: '#4ade80', border: '#166534' },
}
const typeColors = {
  'Registro faltante en contabilidad': '#f87171',
  'Duplicado en contabilidad': '#fb923c',
  'Suplemento no reflejado': '#fbbf24',
  'Comisión incorrecta': '#a78bfa',
  'Diferencia de timing': '#67e8f9',
  'Redondeo': '#94a3b8',
}
const typeIcons = {
...
const resLabels = {
  ajustado: { label: 'Ajustado', color: '#4ade80' },
  aceptado: { label: 'Aceptado', color: '#67e8f9' },
  investigar: { label: 'En investigación', color: '#fbbf24' },
}
...
export default function Dashboard({ data, onReset }) {
  ...
  const cs = { // common styles
    page: { minHeight: '100vh', background: '#080d1a', padding: '32px 20px' },
    wrap: { maxWidth: '1000px', margin: '0 auto' },
    panel: { background: '#0f1729', borderRadius: '12px', border: '1px solid #1e293b', padding: '24px', marginBottom: '16px' },
    lbl: { fontSize: '11px', textTransform: 'uppercase', letterSpacing: '1.5px', color: '#64748b', marginBottom: '6px' },
    sel: { background: '#0f1729', color: '#cbd5e1', border: '1px solid #1e293b', borderRadius: '8px', padding: '7px 12px', fontSize: '13px' },
  }
    ...
    <div style={cs.page}>
      <div style={cs.wrap}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            <div style={{ fontSize: '11px', letterSpacing: '3px', textTransform: 'uppercase', color: '#f87171', fontWeight: 700, marginBottom: '6px' }}>
              Cierre Contable — Análisis Automático
            </div>
            <h1 style={{ fontSize: '26px', fontWeight: 300, margin: 0, color: '#fff' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            {['cuadre', 'detalle'].map(v => (
              <button key={v} onClick={() => setView(v)} style={{
                ...
                border: `1px solid ${view === v ? '#e94560' : '#1e293b'}`,
                background: view === v ? '#e9456022' : '#0f1729',
        ...
        {view === 'cuadre' && <>
          {/* Status banner */}
          <div style={{
            background: cuadra ? '#0a2918' : '#2d121588',
            border: `2px solid ${cuadra ? '#166534' : '#991b1b'}`,
            ...
            <div style={{ position: 'relative', flexShrink: 0 }}>
              ...
              <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center' }}>
                <div style={{ fontSize: '26px', fontWeight: 800, color: cuadra ? '#4ade80' : '#f87171' }}>{pctRes}%</div>
              </div>
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '22px', fontWeight: 700, color: cuadra ? '#4ade80' : '#f87171', marginBottom: '4px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px', marginBottom: '16px' }}>
            {[
              ...
              { l: 'Diferencia bruta', v: fmt(Math.abs(data.diferencia_bruta)), s: `${data.total_discrepancias} partidas`, a: '#fbbf24' },
            ].map(({ l, v, s, a }) => (
              <div key={l} style={cs.panel}>
                <div style={cs.lbl}>{l}</div>
                <div style={{ fontSize: '22px', fontWeight: 700, color: a || '#e2e8f0' }}>{v}</div>
                <div style={{ fontSize: '12px', color: '#475569', marginTop: '2px' }}>{s}</div>
              </div>
            ))}
          </div>
          ...
          <div style={cs.panel}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginTop: '12px' }}>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '32px', fontWeight: 800, color: '#f87171' }}>{pending.length}</div>
                <div style={{ fontSize: '12px', color: '#64748b' }}>Pendientes</div>
                <div style={{ fontSize: '13px', color: '#94a3b8' }}>{fmt(diffPend)}</div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '32px', fontWeight: 800, color: '#4ade80' }}>{resolved.length}</div>
                <div style={{ fontSize: '12px', color: '#64748b' }}>Resueltos</div>
                <div style={{ fontSize: '13px', color: '#94a3b8' }}>{fmt(diffRes)}</div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '32px', fontWeight: 800, color: '#e2e8f0' }}>{items.length}</div>
                <div style={{ fontSize: '12px', color: '#64748b' }}>Total</div>
                <div style={{ fontSize: '13px', color: '#94a3b8' }}>{fmt(diffAll)}</div>
              </div>
            </div>
            <div style={{ height: '8px', background: '#1e293b', borderRadius: '4px', overflow: 'hidden', marginTop: '20px' }}>
              <div style={{ height: '100%', width: `${pctRes}%`, background: 'linear-gradient(90deg, #4ade80, #22d3ee)', borderRadius: '4px', transition: 'width 0.5s ease' }} />
            </div>
          </div>
          ...
          <div style={cs.panel}>
            ...
            {[...new Set(items.map(d => d.tipo))].map(tipo => {
              ...
              const c = typeColors[tipo] || '#94a3b8'
              return (
                <div key={tipo} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '8px 0', borderBottom: '1px solid #1e293b22' }}>
                  <span style={{ fontSize: '15px', color: c, fontWeight: 700, width: '24px', textAlign: 'center' }}>{typeIcons[tipo] || '?'}</span>
                  <span style={{ fontSize: '13px', color: '#cbd5e1', flex: 1 }}>{tipo}</span>
                  <span style={{ fontSize: '12px', color: '#4ade80', fontWeight: 600 }}>{res} resuelto{res !== 1 ? 's' : ''}</span>
                  <span style={{ fontSize: '12px', color: pend > 0 ? '#f87171' : '#475569', fontWeight: 600 }}>{pend} pend.</span>
                </div>
              )
            })}
          </div>
          ...
          <button onClick={() => setView('detalle')} style={{
            ...
            background: '#e9456022', color: '#e94560', cursor: 'pointer', fontSize: '14px', fontWeight: 700,
          }}>
        ...
        {view === 'detalle' && <>
          ...
          <div style={{ display: 'flex', gap: '12px', marginBottom: '16px', flexWrap: 'wrap' }}>
            {[
              { l: 'Pendientes', v: pending.length, c: '#f87171' },
              { l: 'Resueltos', v: resolved.length, c: '#4ade80' },
              { l: 'Importe pendiente', v: fmt(diffPend), c: '#fbbf24', isStr: true },
            ].map(({ l, v, c, isStr }) => (
          ...
          <div style={{ display: 'flex', gap: '10px', marginBottom: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <div style={{ fontSize: '13px', color: '#475569', marginLeft: 'auto' }}>{filtered.length} partida{filtered.length !== 1 ? 's' : ''}</div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            {filtered.map(d => {
              ...
              const tc = typeColors[d.tipo] || '#94a3b8'
              ...
              return (
                <div key={d.id} onClick={() => setExpanded(isOpen ? null : d.id)} style={{
                  background: isRes ? '#0a1a12' : (isOpen ? '#111d36' : '#0f1729'),
                  border: `1px solid ${isRes ? '#16653444' : (isOpen ? tc + '44' : '#1e293b')}`,
                  borderRadius: '10px', padding: '14px 18px', cursor: 'pointer',
                  opacity: isRes && !isOpen ? 0.6 : 1, transition: 'all 0.2s ease',
                }}>
                  {/* Row */}
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                    {isRes && <span style={{ color: resLabels[d.resolucion]?.color || '#4ade80' }}>✓</span>}
                    ...
                    <span style={{ fontSize: '13px', fontWeight: 600, color: '#e2e8f0' }}>{d.poliza}</span>
                    ...
                    {d.diferencia !== 0 && d.diferencia != null && (
                      <span style={{ fontSize: '13px', fontWeight: 700, color: d.diferencia > 0 ? '#4ade80' : '#f87171', marginLeft: 'auto' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '14px' }}>
                        {[
                          ...
                          <div key={l}>
                            ...
                            <div style={{ fontSize: bold ? '14px' : '13px', fontWeight: bold ? 600 : 400, color: '#cbd5e1' }}>{v}</div>
                          </div>
                        ))}
                      </div>

                      <div style={{ background: '#0a1020', borderRadius: '8px', padding: '12px 14px', marginBottom: '8px' }}>
                        ...
                        <div style={{ fontSize: '13px', color: '#94a3b8', lineHeight: 1.6 }}>{d.explicacion}</div>
                      </div>
                      <div style={{ background: '#0a1020', borderRadius: '8px', padding: '12px 14px', marginBottom: '14px' }}>
                        <div style={{ fontSize: '10px', textTransform: 'uppercase', letterSpacing: '1.5px', color: '#4ade80', marginBottom: '4px', fontWeight: 700 }}>Acción recomendada</div>
                        <div style={{ fontSize: '13px', color: '#94a3b8', lineHeight: 1.6 }}>{d.accion_recomendada}</div>
                      </div>

                      {!isRes ? (
                        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                          {[
                            { k: 'ajustado', label: 'Marcar como Ajustado', bc: '#166534', bg: '#0a291844', c: '#4ade80' },
                            { k: 'aceptado', label: 'Aceptar diferencia', bc: '#155e75', bg: '#0a202844', c: '#67e8f9' },
                            { k: 'investigar', label: 'En investigación', bc: '#92400e', bg: '#2d221244', c: '#fbbf24' },
                          ].map(({ k, label, bc, bg, c }) => (
        ...
        <div style={{ marginTop: '32px', textAlign: 'center', fontSize: '12px', color: '#334155' }}>
